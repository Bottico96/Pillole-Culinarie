...
export default function Progetti() {
  const { progetti, professionisti, clienti, loading } = useData()
      ...
      {showModal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal" style={{ maxWidth: 720 }}>
              <div className="modal-header"><span className="modal-title">{editingId ? 'Modifica Progetto' : 'Nuovo Progetto'}</span><button onClick={() => setShowModal(false)} style={{background:"none",border:"none",color:"rgba(255,255,255,.7)",fontSize:20,cursor:"pointer"}}>✕</button></div>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                {clienti && clienti.length > 0
                  ? <select value={form.cliente} onChange={e => { setForm(f => ({ ...f, cliente: e.target.value })); setFieldErrors(fe => ({ ...fe, cliente: null })) }} style={fieldErrors.cliente ? { borderColor: 'var(--red)' } : {}}>
                      <option value="">— Seleziona cliente —</option>
                      {[...clienti].sort((a, b) => (a.nome || '').localeCompare(b.nome || '')).map(c => (
                        <option key={c.id} value={c.nome}>{c.nome}{c.ragioneSociale && c.ragioneSociale !== c.nome ? ` — ${c.ragioneSociale}` : ''}</option>
                      ))}
                    </select>
                  : <input value={form.cliente} onChange={e => { setForm(f => ({ ...f, cliente: e.target.value })); setFieldErrors(fe => ({ ...fe, cliente: null })) }} placeholder="Nessun cliente — aggiungine uno prima" style={fieldErrors.cliente ? { borderColor: 'var(--red)' } : {}} />
                }
                <FieldError error={fieldErrors.cliente} />
              </div>
            </div>
            <div className="form-row">
